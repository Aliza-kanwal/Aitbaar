    ...
    if (invoiceFile) {
      ...
      reader.onload = function(e) {
        try {
          ...
          const isSuspicious = suspiciousPatterns.some(pattern => fileContent.includes(pattern));
          
          if (isSuspicious) {
            alert(
              language === 'ur' 
                ? "⚠️ سیکیورٹی الرٹ: سسٹم نے غیر متعلقہ ڈایاگرام یا سافٹ ویئر فائل کا سراغ لگایا ہے۔ براہ کرم دکان کی اصل رسید (Receipt) اپ لوڈ کریں۔" 
                : "⚠️ AI Fraud Prevention Alert: The embedded OCR scanner detected structural diagram metadata within this image instead of a grocery ledger invoice. Please upload a real commercial receipt."
            );
          ...
          <div className="bg-[#091224] border border-slate-900 w-full max-w-sm rounded-2xl p-6 shadow-2xl relative">
            ...
            {!isLoanDisbursing && !loanSuccess && (
              <>
                ...
                <p className="text-xs text-slate-400 mb-6 leading-relaxed">You are executing an instant loan withdrawal request under the UBL Fin-Trust Credit facility mapping scheme.</p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setIsLoanModalOpen(false)} className="bg-slate-950 text-slate-400 py-3 rounded-xl text-xs font-bold border border-slate-900 hover:text-white transition-all">Cancel</button>
                  <button onClick={() => {
                    setIsLoanDisbursing(true);
                    setTimeout(() => { setIsLoanDisbursing(false); setLoanSuccess(true); }, 1800);
                  }} className="bg-[#00D4B2] text-black py-3 rounded-xl text-xs font-black shadow-lg transition-all">Confirm & Disburse</button>
                </div>
              </>
            )}
            ...
            {isLoanDisbursing && (
              <div className="text-center py-8">
                <div className="w-10 h-10 border-2 border-t-[#00D4B2] border-slate-800 rounded-full animate-spin mx-auto mb-4"></div>
                <h4 className="font-bold text-xs text-slate-400 font-mono tracking-wider uppercase">Connecting Wallet Gateway...</h4>
              </div>
            )}
            ...
            {loanSuccess && (
              <div className="text-center py-6">
                ...
                <p className="text-xs text-slate-400 mt-2 leading-relaxed">The amount of <strong>PKR {dashboardData.limit}</strong> has been transferred safely to your registered wallet account.</p>
                <button onClick={() => { setIsLoanModalOpen(false); setLoanSuccess(false); }} className="mt-6 w-full bg-slate-950 text-slate-300 text-xs py-3 rounded-xl border border-slate-900 hover:text-white transition-all">Done</button>
              </div>
            )}
